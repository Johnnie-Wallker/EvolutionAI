...
def run_model(model_name: str, quantize: str, FT_root: str) -> None:
    ...
    # Load dataset
    with open("data/dataset_2/test.json", "r", encoding="utf-8") as f:
    ...
    if not quantize:
        if not FT_root:
            out_dir = os.path.join("results", "dataset2", "default", model_name)
        else:
            out_dir = os.path.join("results", f"dataset2_{FT_root}", "default", model_name)
    else:
        if not FT_root:
            out_dir = os.path.join("results", "dataset2", quantize, model_name)
        else:
            out_dir = os.path.join("results", f"dataset2_{FT_root}", quantize, model_name)
    ...
    for data in pbar:
        ...
        for page in range(len(images)):
            ...
            out_path = os.path.join(out_dir, f"{folder_name}_{page}.json")
