...
def run_finetune(quantize, epochs, train_batch_size, eval_batch_size, r, alpha, output_dir, train_size, eval_size):
    ...
    def load_custom_docs_d1(json_path):
        """
        Load and process the dataset from the first source (D1).
        """
        ...
        for data in docs:
            if len(data.get("images", [])) != 1:
                continue
            default_fields = [f for f in data.get("fields", []) if f.get("group_name") == "default"]
            if not default_fields:
                continue

            folder = data["hash"]
            image_hash = data["images"][0]["hash"]
            img_path = f"data/dataset_1/imgs/{folder}/{image_hash}.png"

            json_template = {f["name"]: "" for f in default_fields}
            template_str = json.dumps(json_template, indent=2)
            prompt = ("Parse the document by filling the template JSON and returning *only* valid JSON "
                      "matching this template exactly (If any field value contains internal double quotes, "
                      "remove those internal double quotes so they don't break the JSON.):\n") + template_str

            target = {f["name"]: f.get("value", "") for f in default_fields}
            target_str = json.dumps(target, indent=2)

            records.append({"image_path": img_path, "prompt": prompt, "target_str": target_str})
        return Dataset.from_list(records)

    def load_custom_docs_d2(json_path):
        """
        Load and process the dataset from the second source (D2).
        """
        with open(json_path, "r", encoding="utf-8") as f:
            docs = json.load(f)["documents"]
        records = []
    ...
    def load_custom_docs_d3(json_path):
        with open(json_path, "r", encoding="utf-8") as f:
            docs = json.load(f)
        records = []
        for data in docs:
            ...
            target_str = json.dumps(fields, indent=2)
            records.append({"image_path": img_path, "prompt": prompt, "target_str": target_str})
        return Dataset.from_list(records)

    # Load and combine training datasets from D1 and D2
    train_d1 = load_custom_docs_d1("data/dataset_1/train.json")
    train_d2 = load_custom_docs_d2("data/dataset_2/train.json")
    train_d3 = load_custom_docs_d3("data/dataset_3/train.json")
    train_combined = concatenate_datasets([train_d1, train_d2, train_d3])

    test_d1 = load_custom_docs_d1("data/dataset_1/test.json")
    test_d2 = load_custom_docs_d2("data/dataset_2/test.json")
    test_d3 = load_custom_docs_d3("data/dataset_3/test.json")
    test_combined = concatenate_datasets([test_d1, test_d2, test_d3])

    # Handle dataset sizing based on user input
    if train_size != "full":
        train_combined = train_combined.shuffle(seed=1).select(range(int(train_size)))
    if eval_size != "full":
        test_combined = test_combined.shuffle(seed=1).select(range(int(eval_size)))
    ...
    # Map the dataset
    train_tok = train_combined.map(preprocess_train, remove_columns=train_combined.column_names,
                                   batched=False, writer_batch_size=200)
    test_tok = test_combined.map(preprocess_eval, remove_columns=test_combined.column_names,
                                 batched=False, writer_batch_size=200)
