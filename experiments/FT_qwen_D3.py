...
def run_finetune(quantize, epochs, train_batch_size, eval_batch_size, r, alpha, output_dir, train_size, eval_size):
    ...
    if train_size == "full":
        train = load_custom_docs("data/dataset_3/train.json")
    else:
        train = load_custom_docs("data/dataset_3/train.json").select(range(int(train_size)))
    if eval_size == "full":
        test = load_custom_docs("data/dataset_3/test.json")
    else:
        test = load_custom_docs("data/dataset_3/test.json").select(range(int(eval_size)))
    ...
    # Map the dataset
    train_tok = train.map(preprocess_train, remove_columns=train.column_names, batched=False, writer_batch_size=50)
    test_tok = test.map(preprocess_eval, remove_columns=test.column_names, batched=False, writer_batch_size=50)
