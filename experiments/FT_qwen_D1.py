...
def run_finetune(quantize, epochs, train_batch_size, eval_batch_size, r, alpha, output_dir, train_size, eval_size):
    ...
    if train_size == "full":
        train = load_custom_docs("data/dataset_1/train.json")
    else:
        train = load_custom_docs("data/dataset_1/train.json").select(range(int(train_size)))
    if eval_size == "full":
        test = load_custom_docs("data/dataset_1/test.json")
    else:
        test = load_custom_docs("data/dataset_1/test.json").select(range(int(eval_size)))
