...
def mask_values(obj):
    """
    Recursively replace every non-dict/list value in obj with an empty string,
    preserving the overall structure.
    """
    if isinstance(obj, dict):
        return {k: mask_values(v) for k, v in obj.items()}
    elif isinstance(obj, list):
        return [mask_values(item) for item in obj]
    else:
        # anything that’s not a dict or list gets replaced
        return ""